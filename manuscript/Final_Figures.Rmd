...
phylos <- lapply(seq_along(tree_data_list), function(i) {
    ...
    ggtree(tree, mrsd = "2016-01-01", color = "black", lwd = 1.3, layout = "roundrect") +
  # geom_nodelab(size = 4, nudge_y = 0.4) +
  # geom_tiplab(size = 4) +
  geom_tippoint(aes(colour = posterior), size = 2, show.legend = F) +
  geom_nodepoint(aes(colour = posterior), size = 2, show.legend = if (i == 5) TRUE else FALSE) +
  scale_color_viridis() +
  labs(title = names(tree_data_list)[i], color = "Posterior\nprobability") +
  theme_tree2() +
            theme(
...
cowplot::save_plot("./Figures/map_trees.jpeg", P_all, base_height = 10, base_width = 20, dpi = 300)
```
