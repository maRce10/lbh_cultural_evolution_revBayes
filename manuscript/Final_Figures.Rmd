...
left_margin <- 0.08
right_margin <- 0.1
cbs <- c(left_margin, ((1 - (left_margin + right_margin)) / 2) + left_margin, 1 - right_margin, 1)

m <- rbind(c(cbs[1], cbs[2], 0.5, 1), # top left
           c(cbs[2], cbs[3], 0.5, 1), # top right
           c(cbs[1], cbs[2], 0, 0.5), # bottom left
           c(cbs[2], cbs[3], 0, 0.5), # bottom right
           c(0, cbs[1], 0, 1), # freq label
           c(cbs[3], 1, 0, 1) # legend
           )
...
par(mar = c(0, 0, 0, 0), bg = adjustcolor("white", alpha.f = 0), new = T, xpd = NA)
          ...
          text(x = 0.5, y = 1, "Frequency (kHz)", srt = 90, cex = 1)
...
for (e in 1:nrow(merge_sgmts)){
  ...
  text(x = 0.015, y = 0.85, labels = "20 ms", cex = 0.65, adj = 0.2, col = "white")
}
