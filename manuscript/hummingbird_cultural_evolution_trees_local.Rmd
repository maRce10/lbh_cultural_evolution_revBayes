...
Bird songs provide a promising system for exploring the suitability of phylogenetic reconstruction methods for animal cultural traits because they are often socially learned and can be composed of sequences of discrete acoustic elements that can be analysed as ordered character strings. The Long-billed Hermit (*Phaethornis longirostris*; Fig. 1a) produces songs that can be represented as sequences of discrete sounds fused together into an unbroken signal (Fig. 2; see 'Methods'). Indeed, the most salient differences among song types reside in the composition and sequential order of their sounds [@Araya-Salas2013]. Evidence of social vocal production learning in this species [*sensu* @ten2021re], includes micro-geographic song variation decoupled from genetic structure [@Araya-Salas2019] and adult replacement of crystallized songs [@Araya-Salas2013]. Males sing a single song-type repertoire, which enables comparisons of individual songs as homologous traits (as opposed to multiple song-type repertoires). Courtship occurs within leks of 5-20 highly vocal males [@Biologla1979], which facilitates longitudinal monitoring of all song types within a lek. Moreover, song types can be shared by sub-groups of males within leks, with no evidence of song type sharing across leks [@Araya-Salas2019], suggesting that leks operate as relatively isolated cultural systems. Such independence across leks provides an unmatched opportunity to investigate the robustness of phylogenetic models across different iterations of an underlying cultural diversification process.
...
Long-billed Hermit songs are composed of two basic sound types: tonal sounds and trills. These different sounds are clearly observed in the frequency contour of the dominant harmonics (Fig. 2). Pure tones can vary in the degree of modulation (i.e. changes in frequency through time), while trills vary in the number of oscillations per unit of time (i.e. rate) (Fig. 2). We subdivided these two basic sound types into six categories (Fig. 2): slow trill, medium-paced trill, fast trill, downward pure tone, upward pure tone and flat pure tone. Songs were split into 20 equal-length segments, and each segment was assigned to one of these six categories, based upon visual inspection of spectrograms (Fig. S1). The choice of 20 segments per song captured a compromise between our ability to discriminate sound categories, which increased with segment length, and the probability that a single sound type occurred in each segment, which decreased with segment length. We assessed the objectivity of sound classification by quantifying inter-observer agreement between two independent observers who classified a subset of songs. Agreement was quantified using Cohen’s kappa coefficient (unweighted), calculated with the kappa2 function in the R package irr [v0.84.1, @gamer2019].
...
Our second alignment strategy also used the L-INS-i algorithm implemented in MAFFT with default gap penalties, but incorporated biological assumptions about how Long-billed Hermit songs are culturally modified. Specifically, we assumed that when individuals modify pre-existing songs they are more likely to replace a trill with a different type of trill and a tone with a different type of tone than to change between trills and tonal sounds. We implemented this assumption by specifying custom substitution costs that penalized mismatches between sound categories (trill vs. pure tone) more strongly than mismatches within categories. To determine biologically realistic substitution costs, we developed an empirical optimization procedure designed to identify mismatch scores that maximize the difference between alignments of culturally related song sequences and alignments of sequences lacking cultural structure. This was achieved by comparing alignments of song types recorded within the same lek with alignments of randomly assembled (i.e. simulated) songs. The rationale is that song types produced within a lek share cultural ancestry and therefore exhibit structural similarities, whereas randomly assembled songs should lack such resemblance. If insertion and deletion events accumulate through time as observed in molecular sequences [@Loeytynoja2012], alignments of culturally related songs should require fewer inferred insertion–deletion events than alignments of unrelated sequences, resulting in shorter alignments with fewer gaps.

To generate a null expectation for unrelated sequences, we simulated song sequences that preserved key properties of the empirical data, including the distribution of segment types and segment lengths (number of segments), but were assembled randomly. For each candidate substitution matrix, we aligned both the empirical  and simulated sequences using MAFFT and calculated the number of gaps introduced by the alignment. We then computed the difference in alignment length between real and simulated data sets and selected substitution costs that maximized this difference. We evaluated substitution costs for three classes of segment comparisons: identical segments, segments belonging to the same acoustic category, and segments belonging to different categories. Candidate substitution matrices were constructed such that costs increased monotonically across these classes, with the lowest scores assigned to substitutions between different acoustic categories, intermediate scores assigned to substitutions within categories, and the highest scores assigned to identical segments. This constraint reflects the assumption that substitutions between trills and tonal sounds are less likely than substitutions within acoustic categories. The substitution costs that maximized the difference in alignment length between real and simulated songs were then used for sequence alignment. We hereafter refer to this alignment strategy as 'MAFFT-optimal'.
...
Our data set of historical records of songs in hummingbird leks has three advantages in comparison to most fossil data sets used in phylogenetic analyses. First, there is no stratigraphic uncertainty. We can be certain that historical songs occurred in the year when they were recorded. Second, there are no partial fossils. Songs recorded in the past are just as complete as the most recent ones, creating no additional ambiguity in character states of historical songs. Third, the historical record is relatively rich. In all leks, there are multiple years sampled consecutively, and in two leks (SUR and CCE) historical records go back to 1969 (Fig. 1c). Because leks are small and Long-billed Hermits are actively displaying their songs [@Biologla1979], we can assume detection is nearly perfect and thus there is no missing taxa in any of the sampled years.
...
\begin{figure}
  ...
  \captionsetup{labelformat=empty}
  \caption{\textbf{Figure 6. } Song element diversity ($\pi$) in empirical song data and posterior predictive data simulations (see Methods). For simulated data, we report the mean and 95$\%$ HPD interval of 1500 (BR1, TR1, HC1) or 3000 (CCE and SUR) simulations. }
  \end{figure}
...
\begin{figure}
  ...
  \captionsetup{labelformat=empty}
  \caption{\textbf{Figure 9. } Age and diversification estimates under different alignment strategies in five leks of the Long-billed Hermit. Histograms show posterior distributions of parameter estimates in models using a relaxed molecular clock and all historical records. All available historical records are included in the analysis. }
  \end{figure}
...
Finally, our study assumes that songs diversify in a tree-like manner, with no horizontal transfer. For the Long-billed Hermit system, this means that juvenile males introduce changes on their learning templates, independently of other songs that they may have heard in the same lek. We do not know if males of the Long-billed Hermit indeed copy and potentially modify a song template, or if new songs can and are often formed by combining elements from multiple templates. **Maybe a sentence here about what we do know about the learning process.** Nonetheless, it is possible that cultural phylogenetic models are robust to a degree of horizontal transfer, as suggested by the relatively high absolute fit of these models to song data (Table 1). An early study based on cladistics showed that data sets across different cultural domains had a similar fit to a tree-like diversification process as biological data sets [REF Collard]. Further assessments of the robustness of cultural phylogenetics to observed levels of horizontal transfer and reticulate evolution are warranted, as well as the exploration of recently developed methods that can accommodate such processes [REF to that french paper].  
