...
#SBATCH --job-name=ML.Approx.R2-long
...
leks=("TR1") # Forgot TR1 and instead repeated "CCE"
    ...
    do
      ...
      do
        ...
        do
          ...
          lekinfo="${j}"."${k}"."${m}"."${n}"."${o}"
          echo $lekinfo
          echo $lekinfo >> ./output/revbayes/stdout/datasetR2.txt
          rb_command="source(\"./"$j"_"$k"_"$m"_"$n""$o".Rev\");"
          ML_out="../output/revbayes/ML/"$j"_"$k"_"$m"_"$n""$o".R2.txt"
          echo $rb_command | mpiexec -np 16 rb-mpi | tee $ML_out
          cat $ML_out | tail -1 >> ../output/revbayes/stdout/PathSamplerR2.txt
          cat $ML_out | tail -2 | head -1 >> ../output/revbayes/stdout/SteppingStoneR2.txt
